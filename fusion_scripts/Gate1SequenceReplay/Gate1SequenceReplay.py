...
def _sketch_plane(component, operation, angular_tolerance):
    ...
    raise ReplayError(
        ...
        "operation_cap replay belongs to the Gate 1 next stage",
    )
...
def _run_sequence(sequence, component, design, world_point, parallel_alignment_error):
    ...
    for operation in sequence["operations"]:
        ...
        if operation_type == "sketch":
            plane = _sketch_plane(component, operation, angular_tolerance)
            ...
            if sketch.profiles.count != 1:
                raise ReplayError(
                    ...
                    "{} produced {} profiles; expected 1".format(
                        operation_id, sketch.profiles.count
                    ),
                )
            sketches[operation_id] = sketch
        elif operation_type == "extrude":
            if (
                operation["boolean_type"] != "new"
                or operation["extent"]["type"] != "distance"
            ):
                raise ReplayError(
                    "unsupported_operation_combination",
                    "this stage supports only new + distance",
                )
            sketch = sketches[operation["profile"]["sketch_id"]]
            actual_normal = _actual_sketch_normal(sketch)
            direction = tuple(float(value) for value in operation["direction"])
            signed_distance = float(operation["extent"]["distance_mm"])
            if _dot(actual_normal, direction) < 0:
                signed_distance = -signed_distance
            component.features.extrudeFeatures.addSimple(
                sketch.profiles.item(0),
                adsk.core.ValueInput.createByReal(_mm_to_internal(units, signed_distance)),
        ...
        operation_records.append({"operation_id": operation_id, "status": "success"})
