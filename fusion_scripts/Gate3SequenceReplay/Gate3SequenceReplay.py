"""Replay Gate 3 development sequences through the validated Gate 2 adapter."""
...
def _load_gate2_adapter(project_root):
    ...
    spec = importlib.util.spec_from_file_location("_gate3_gate2_adapter_current", path)
...
def _paths(project_root, request):
    if request.get("request_version") != "gate3-replay-0.1":
    ...
    if not isinstance(run_id, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]*", run_id):
        raise ReplayError("invalid_run_id", "invalid Gate 3 run ID")
    ...
    if (
        ...
        raise ReplayError(
            "invalid_case_set",
            "request must contain D-H01..D-H05 exactly once with D-H04 first",
        )
    cases = {}
...
def run(context):
    ...
    request_path = os.path.join(project_root, "config", "gate3_replay_request.json")
    paths = None
    records = []
    try:
        ...
        ui.messageBox(
            "Gate 3 batch complete: {} success, {} failure".format(
                batch["success_count"], batch["failure_count"]
            )
        )
    except Exception as exc:
        ui.messageBox("Gate 3 batch failed [{}]:\n{}".format(_error_code(exc), exc))
